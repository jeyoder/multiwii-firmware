...
#else
  volatile uint16_t rcValue[RC_CHANS] = {0, 0, 0, 0, 0, 0, 0, 0}; // interval [1000;2000]
#endif
...
void computeRC() {
  ...
  uint8_t chan,a;
    ...
    for (chan = 0; chan < RC_CHANS; chan++) {
        ...
        if ( rcDataMean[chan] > (uint16_t)rcData[chan] +3)  rcData[chan] = rcDataMean[chan]-2;
      #endif
      if (chan<8 /*&& rcSerialCount > 0*/) { // rcData comes from MSP and overrides RX Data until rcSerialCount reaches 0 // OVERRIDING. ALWAYS USE RC SERIAL
        if(rcSerialCount > 0) rcSerialCount --;
        #if defined(FAILSAFE)
          if(rcSerialCount > 0) failsafeCnt = 0; //kick the failsafe watchdog if serial RX is alive
        #endif
